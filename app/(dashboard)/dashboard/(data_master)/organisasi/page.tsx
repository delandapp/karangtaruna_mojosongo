...
import { OrganisasiFormModal } from "@/components/organisms/modals/OrganisasiFormModal";
...
export default function OrganisasiPage() {
    ...
    const handleDelete = async (id: number, nama: string) => {
        if (!window.confirm(`Apakah Anda yakin ingin menghapus data organisasi ${nama}?`)) return;

        try {
            await deleteOrganisasi(id).unwrap();
            toast.success("Berhasil dihapus", { description: `Data organisasi ${nama} telah dihapus.` });
        } catch (error: any) {
            toast.error("Gagal menghapus", { 
                description: error?.data?.error?.message || "Terjadi kesalahan saat menghapus data." 
            });
        }
    };
                    ...
                    <div className="mb-6 flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-muted-foreground">
                                Kelola direktori profil berbagai organisasi Karang Taruna di tingkat wilayah.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        {isLoading ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full">
                                {[1, 2, 3, 4].map((i) => (
                                    <div key={i} className="h-[360px] animate-pulse bg-card/50 rounded-xl border border-border/50"></div>
                                ))}
                            </div>
                            ...
                            <div className="rounded-xl border border-border/50 bg-card/80 shadow-sm backdrop-blur flex flex-col w-full items-center justify-center p-12 py-32">
                                ...
                                <h3 className="text-xl font-semibold text-foreground">Tidak ada data Organisasi</h3>
                                ...
                                {!search && (
                                    <Button variant="outline" className="mt-6 border-border/60" onClick={handleOpenCreate}>
                        ...
                        {meta && meta.totalPages > 1 && (
                            <div className="flex items-center justify-between border-t border-border/50 px-4 py-4 sm:px-6 bg-card/50 rounded-xl mt-4">
                                <p className="text-sm text-muted-foreground hidden sm:block">
                                    Menampilkan <span className="font-medium text-foreground">{(page - 1) * 12 + 1}</span> hingga{" "}
                                    <span className="font-medium text-foreground">{Math.min(page * 12, meta.total)}</span> dari{" "}
                                    <span className="font-medium text-foreground">{meta.total}</span> organisasi
                                </p>
                                <CustomPagination
                                    currentPage={page}
