...
export const GET = withAuth(async (req: AuthenticatedRequest) => {
    try {
        ...
        const cacheKey = `hak_akses:all:page:${page}:limit:${limit}`;
        if (!searchQuery) {
        ...
        const { hits: hakAkses, total } = await searchDocuments(
            ELASTIC_INDICES.HAK_AKSES,
            esQuery,
            { from: skip, size: limit, sort: [{ endpoint: "asc" }, { method: "asc" }] },
        );
