...
export const GET = withAuth(async (req: AuthenticatedRequest) => {
  try {
    ...
    if (dropdown) {
      ...
      const items = await prisma.m_skala_perusahaan.findMany({
    ...
    const [items, total] = await Promise.all([
      prisma.m_skala_perusahaan.findMany({
        ...
        skip,
        take: limit,
      }), // Catatan: m_skala_perusahaan tidak memiliki dibuat_pada/updatedAt pada schema yg diberikan
      prisma.m_skala_perusahaan.count({ where: whereCondition }),
    ]);
...
export const POST = withAuth(async (req: AuthenticatedRequest) => {
  try {
    ...
    const singleCacheKey = REDIS_KEYS.SKALA_PERUSAHAAN.SINGLE(newItem.id);
