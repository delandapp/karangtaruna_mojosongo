...
import { HakAksesFormModal } from "./HakAksesFormModal";
import { useGetHakAksesQuery, useDeleteHakAksesMutation } from "@/features/api/hakAksesApi";
...
export function HakAksesTable() {
    ...
    const [editingHakAkses, setEditingHakAkses] = useState<HakAksesData | null>(null);
    ...
    const handleDelete = async (id: number, fitur: string, method: string) => {
        if (
            !window.confirm(`Apakah Anda yakin ingin menghapus hak akses [${method}] ${fitur}?`)
        )
            return;
                                ...
                                >
                                    ...
                                    <TableCell>
                                        <div className="flex flex-wrap gap-1">
                                            {item.is_all_level ? (
                                                <Badge variant="secondary" className="bg-primary/10 text-primary">Semua Level</Badge>
                                            ) : item.rules && item.rules.filter(r => r.level).length > 0 ? (
                                                Array.from(new Set(item.rules.filter(r => r.level).map(r => r.level?.nama_level))).map(levelName => (
                                                    <Badge key={levelName} variant="secondary" className="bg-muted text-muted-foreground">{levelName}</Badge>
                                                ))
                                            ) : (
                                    ...
                                    <TableCell>
                                        <div className="flex flex-wrap gap-1">
                                            {item.is_all_jabatan ? (
                                                <Badge variant="secondary" className="bg-primary/10 text-primary">Semua Jabatan</Badge>
                                            ) : item.rules && item.rules.filter(r => r.jabatan).length > 0 ? (
                                                Array.from(new Set(item.rules.filter(r => r.jabatan).map(r => r.jabatan?.nama_jabatan))).map(jabatanName => (
                                                    <Badge key={jabatanName} variant="outline" className="bg-background text-foreground">{jabatanName}</Badge>
                                                ))
                                            ) : (
                                                <span className="text-sm text-muted-foreground">-</span>
                                            )}
