...
export const TabEProposal: React.FC<TabEProposalProps> = ({ eventId }) => {
  const {
    ...
    refetch,
  } = useGetEProposalByEventIdQuery(eventId);
  ...
  useEffect(() => {
    if (proposal) {
      ...
      if (proposal.daftar_isi?.length) {
        setTocEntries(
          [...proposal.daftar_isi].sort((a, b) => a.urutan - b.urutan),
        );
      }
    } else {
    ...
    try {
      ...
      if (pdfFile) {
        const res = await uploadFile({
          bucketName: S3_BUCKETS.E_PROPOSAL,
          file: pdfFile,
        }).unwrap();
      ...
      if (coverFile) {
        ...
        if (!allowedCoverTypes.includes(coverFile.type)) {
          toast.error("Gagal mengunggah", {
            description: "Format cover tidak valid! Hanya JPEG dan PNG yang diizinkan.",
          });
          return;
        }
        const res = await uploadFile({
          bucketName: S3_BUCKETS.COVER_E_PROPOSAL,
          file: coverFile,
        }).unwrap();
        if (res.success === false) throw res;
        cover_url =
      ...
      let bg_music_url = proposal?.pengaturan?.bg_music_url || "";
      if (musicFile) {
        const allowedMusicTypes = ["audio/mpeg"];
        if (!allowedMusicTypes.includes(musicFile.type)) {
          toast.error("Gagal mengunggah", {
            description: "Format musik tidak valid! Hanya file MP3 yang diizinkan.",
          });
          return;
        }
        const res = await uploadFile({
          bucketName: S3_BUCKETS.SOUND_E_PROPOSAL,
          file: musicFile,
        }).unwrap();
        if (res.success === false) throw res;
        bg_music_url =
      ...
      if (!file_pdf_url) {
        toast.error("Validasi gagal", {
          description: "File PDF Proposal wajib diunggah!",
        });
        return;
      }
      ...
      const payload = {
        ...
        daftar_isi: tocEntries.map(({ judul, halaman, urutan }) => ({
          judul,
          halaman,
          urutan,
        })),
      ...
      if (proposal) {
        ...
        toast.success("Berhasil diperbarui", {
          description: "Data E-Proposal telah diperbarui.",
        });
      } else {
        await createProposal(payload).unwrap();
        toast.success("Berhasil ditambahkan", {
          description: "Data E-Proposal berhasil dibuat.",
        });
      }
      refetch();
    } catch (err: any) {
      ...
      toast.error("Gagal menyimpan data", {
        description:
          ...
          err?.data?.message ||
          err?.message ||
          "Terjadi kesalahan saat upload atau menyimpan.",
      });
    }
  };
  ...
  const saveTocEntry = () => {
    if (!tocForm.judul.trim()) {
      toast.error("Validasi gagal", {
        description: "Judul tidak boleh kosong.",
      });
      return;
    }
    if (tocForm.halaman < 1) {
      toast.error("Validasi gagal", {
        description: "Halaman harus minimal 1.",
      });
      return;
    }
    ...
    if (editingToc) {
      setTocEntries((prev) =>
        prev.map((e) =>
          e === editingToc
            ? { ...e, judul: tocForm.judul, halaman: tocForm.halaman }
            : e,
        ),
      );
    } else {
      const maxUrutan =
        tocEntries.length > 0
          ? Math.max(...tocEntries.map((e) => e.urutan))
          : 0;
      setTocEntries((prev) => [
        ...prev,
        {
          judul: tocForm.judul,
          halaman: tocForm.halaman,
          urutan: maxUrutan + 1,
        },
      ]);
    }
  ...
  const moveTocEntry = (index: number, direction: "up" | "down") => {
    ...
    newEntries[index] = {
      ...newEntries[index],
      urutan: newEntries[swapIdx].urutan,
    };
    newEntries[swapIdx] = { ...newEntries[swapIdx], urutan: tempUrutan };
    // Swap positions
    [newEntries[index], newEntries[swapIdx]] = [
      newEntries[swapIdx],
      newEntries[index],
    ];
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription>
                Unggah dan atur tampilan presentasi interaktif proposal Anda.
              </CardDescription>
            </div>
            {proposal && (
              <Button
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="judul"
                    {...form.register("judul", { required: true })}
                    placeholder="Proposal Sponsorship..."
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="event-keren-2026"
                  />
                  <p className="text-xs text-muted-foreground">
                    Proposal dapat diakses melalui: /proposal/
                    <b>{form.watch("slug") || "..."}</b>
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Deskripsi untuk dibagikan di sosial media..."
                    rows={3}
                  />
                </div>
                ...
                <div className="space-y-2 pt-2 border-t mt-4">
                  ...
                  {proposal?.file_pdf_url && !pdfFile && (
                    <p className="text-xs text-green-600 mt-1">
                      ✓ PDF aktif saat ini:{" "}
                      {proposal.file_pdf_url.split("/").pop()}
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-2 pt-2">
                  ...
                  <Input
                    ...
                    title="Pilih file gambar cover (JPEG/PNG)"
                    onChange={(e) => {
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Format yang diizinkan: JPEG, PNG
                  </p>
                  {proposal?.cover_url && !coverFile && (
                    <p className="text-xs text-green-600 mt-1">
                      ✓ Cover aktif saat ini:{" "}
                      {proposal.cover_url.split("/").pop()}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-3 border rounded-lg bg-stone-50/50 dark:bg-stone-900/50">
                  <div className="space-y-0.5">
                    <Label className="text-base">
                      Efek Suara (Halaman Beralih)
                    </Label>
                    <p className="text-xs text-muted-foreground">
                      Memutar suara ketika membalik halaman.
                    </p>
                  </div>
                  <input
                    type="checkbox"
                    ...
                    onChange={(e) =>
                      form.setValue("sound_effect", e.target.checked)
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between p-3 border rounded-lg bg-stone-50/50 dark:bg-stone-900/50">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Berpindah halaman otomatis setiap beberapa detik.
                    </p>
                  </div>
                  <input
                    type="checkbox"
                    ...
                    onChange={(e) =>
                      form.setValue("auto_flip", e.target.checked)
                    }
                  />
                </div>
                ...
                <div className="space-y-2 pt-2">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <Label>Background Music (Opsional)</Label>
                  </div>
                  <Input
                    type="file"
                    accept="audio/mpeg"
                    title="Pilih file musik latar (MP3)"
                    onChange={(e) => {
                      const file = e.target.files?.[0] || null;
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Unggah file MP3 yang akan diputar pelan sebagai latar
                    belakang.
                  </p>
                  {proposal?.pengaturan?.bg_music_url && !musicFile && (
                    <p className="text-xs text-green-600 mt-1">
                      ✓ Musik aktif saat ini:{" "}
                      {proposal.pengaturan.bg_music_url.split("/").pop()}
                    </p>
                  )}
                </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <Dialog open={tocDialogOpen} onOpenChange={setTocDialogOpen}>
                  ...
                  <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                      <DialogTitle>
                        {editingToc ? "Edit Daftar Isi" : "Tambah Daftar Isi"}
                      </DialogTitle>
                      <DialogDescription>
                        Tentukan judul bab/bagian dan nomor halaman di PDF.
                      </DialogDescription>
                    </DialogHeader>
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setTocForm((prev) => ({
                              ...prev,
                              halaman: parseInt(e.target.value) || 1,
                            }))
                          }
                        />
                        <p className="text-xs text-muted-foreground">
                          Halaman sesuai nomor halaman di file PDF (dimulai dari
                          1).
                        </p>
                      </div>
                    </div>
              ...
              {tocEntries.length > 0 ? (
                ...
                <div className="border rounded-lg p-8 text-center">
                  ...
                  <p className="text-xs text-muted-foreground/70 mt-1">
                    Tambahkan bab atau bagian untuk navigasi cepat di flipbook.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex justify-end pt-4 border-t mt-6">
              <Button
                type="submit"
                disabled={isSaving}
                className="min-w-[150px] shadow-primary/30 shadow-lg hover:shadow-primary/50 transition-all"
              >
                {isSaving ? (
                  <>
