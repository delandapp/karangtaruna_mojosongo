...
import {
  Loader2, X, Upload, ImageIcon, Eye,
  Newspaper, Image as ImageLucide, Search,
  Tag, Hash, Star, Zap, Calendar, ArrowLeft,
} from "lucide-react";
...
const S3_BUCKET = S3_BUCKETS.BERITA || "assets-berita-karang-taruna";
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        ...
        className={cn(
          "relative flex min-h-[220px] flex-col items-center justify-center rounded-xl border-2 border-dashed transition-all",
      ...
      >
        {preview ? (
          ...
          <label className="flex flex-col items-center gap-3 cursor-pointer p-8 text-center w-full h-full justify-center">
            {isLoading ? (
              <>
...
export function BeritaForm({ initialData }: BeritaFormProps) {
  ...
  const handleSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      toast.success(isEditing ? "Berita berhasil diperbarui" : "Berita berhasil disimpan sebagai draft", {
        description: `"${values.judul}" telah disimpan.`,
      });
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="flex flex-col">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col w-full">
            <div className="px-6 pt-5 pb-0 shrink-0">
              <TabsList className="h-10 gap-1 bg-muted/60 p-1 w-full max-w-md">
            ...
            {/* ── Tab: Konten ── */}
            <TabsContent value="konten" className="mt-0 outline-none">
              <div className="space-y-6 px-6 pb-6">
                ...
                <div className="space-y-4">
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          ...
                          className="bg-muted/40 text-base font-semibold focus-visible:ring-primary/50 border-border/60"
                          {...field}
                        />
                      </FormControl>
                  ...
                  <FormField control={form.control} name="sub_judul" render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm text-muted-foreground font-medium">Sub-judul / Ringkasan</FormLabel>
                      <FormControl>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <FormField control={form.control} name="penulis" render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-xs font-semibold">Penulis <span className="text-destructive">*</span></FormLabel>
                      <FormControl>
                  ...
                  <FormField control={form.control} name="editor" render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-xs font-semibold">Editor</FormLabel>
                      <FormControl>
                  ...
                  <FormField control={form.control} name="m_kategori_berita_id" render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-xs font-semibold">Kategori <span className="text-destructive">*</span></FormLabel>
                ...
                <div className="space-y-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <FormLabel className="text-xs font-semibold text-muted-foreground">Tag Berita</FormLabel>
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <FormField control={form.control} name="is_featured" render={({ field }) => (
                    <FormItem className="flex items-center gap-3 rounded-xl border border-border/50 bg-muted/30 px-4 py-3 space-y-0">
                      ...
                      <div>
                        <FormLabel className="text-sm font-semibold cursor-pointer">Berita Unggulan</FormLabel>
                  ...
                  <FormField control={form.control} name="is_breaking_news" render={({ field }) => (
                    <FormItem className="flex items-center gap-3 rounded-xl border border-border/50 bg-muted/30 px-4 py-3 space-y-0">
                      ...
                      <div>
                        <FormLabel className="text-sm font-semibold cursor-pointer">Breaking News</FormLabel>
            ...
            {/* ── Tab: Cover ── */}
            <TabsContent value="cover" className="mt-0 outline-none">
              <div className="space-y-6 px-6 pb-6 max-w-2xl">
                <div>
                ...
                {coverUrl && (
                  <div className="rounded-xl border border-border/50 overflow-hidden bg-card/65">
                    <div className="bg-muted/40 px-3 py-2 flex items-center gap-2 border-b border-border/40">
                      <Eye className="h-3.5 w-3.5 text-muted-foreground" />
                      <span className="text-xs font-semibold text-muted-foreground">Preview Card</span>
                    </div>
                    <div className="p-4">
                      <div className="overflow-hidden rounded-lg max-w-md border border-border/50">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={coverUrl} alt="Preview" className="w-full object-cover" style={{ height: "180px" }} />
                      </div>
                    </div>
                  </div>
                )}
            ...
            <TabsContent value="seo" className="mt-0 outline-none">
              <div className="space-y-6 px-6 pb-6">
                ...
                {/* SEO Preview */}
                <div className="rounded-xl border border-border/50 bg-muted/20 p-5 max-w-3xl">
                  <p className="mb-3 text-xs font-semibold text-muted-foreground">Preview Google Search</p>
                  <div className="space-y-1">
                    <p className="text-xs text-muted-foreground">karangtarunamojosongo.id › berita › {seoSlug || "slug-berita"}</p>
                    <p className="text-base font-semibold text-blue-600 dark:text-blue-400 line-clamp-1">
                      {seoTitle || judul || "Judul Berita"}
                    </p>
                    <p className="text-sm text-muted-foreground line-clamp-2">
                      {seoDesc || "Deskripsi SEO akan tampil di sini. Tulis deskripsi menarik untuk meningkatkan klik di hasil pencarian."}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <FormField control={form.control} name="seo_slug" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription className="text-xs">
                        /berita/<span className="text-primary font-medium">{field.value || "..."}</span>
                      </FormDescription>
                      <FormMessage className="text-xs" />
                    </FormItem>
                  )} />
                  ...
                  <FormField control={form.control} name="seo_title" render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm font-semibold">
                        Judul SEO
                        <span className={cn("ml-2 text-xs font-normal", (field.value?.length || 0) > 70 ? "text-destructive" : "text-muted-foreground")}>
                ...
                <FormField control={form.control} name="seo_description" render={({ field }) => (
                  <FormItem className="max-w-3xl">
                    <FormLabel className="text-sm font-semibold">
                      Deskripsi SEO
                      <span className={cn("ml-2 text-xs font-normal", (field.value?.length || 0) > 160 ? "text-destructive" : "text-muted-foreground")}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <FormField control={form.control} name="seo_robots" render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm font-semibold">Robots</FormLabel>
                ...
                <FormField control={form.control} name="seo_og_image_url" render={({ field }) => (
                  <FormItem className="max-w-3xl">
                    <FormLabel className="text-sm font-semibold">OG Image URL</FormLabel>
                    <FormControl>
          ...
          <div className="shrink-0 border-t border-border/50 bg-muted/20 px-6 py-4 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              <Button
                ...
                className="bg-transparent border-border/60 hover:bg-muted/50"
              >
                Batal
              </Button>
              <Button
