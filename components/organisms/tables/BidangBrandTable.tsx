...
import { BidangBrandFormModal } from "../modals/sponsorship/BidangBrandFormModal";
...
export function BidangBrandTable() {
  ...
  const [editingBidangBrand, setEditingBidangBrand] =
    useState<BidangBrandData | null>(null);
  ...
  const bidangBrands = response?.data || [];
  const total = response?.meta?.totalPages || 0;

  const fetchBidangBrands = () => {
    refetch();
  };

  const [deleteBidangBrand] = useDeleteBidangBrandMutation();

  const handleDelete = async (id: number, nama: string) => {
    if (
      !window.confirm(`Apakah Anda yakin ingin menghapus bidang brand ${nama}?`)
    )
      return;

    try {
      await deleteBidangBrand(id).unwrap();
      toast.success("Berhasil dihapus", {
        description: `Bidang brand ${nama} telah dihapus.`,
      });
    } catch (error: any) {
  ...
  const handleEdit = (bidangBrand: BidangBrandData) => {
    setEditingBidangBrand(bidangBrand);
    setIsModalOpen(true);
  };

  const handleCreate = () => {
    setEditingBidangBrand(null);
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="flex flex-col gap-4">
              ...
              <TableRow>
                <TableCell colSpan={3} className="h-32 text-center">
                  <div className="flex flex-col items-center justify-center text-muted-foreground">
                    ...
                    <span>Memuat data bidang brand...</span>
                  </div>
                </TableCell>
              </TableRow>
            ) : bidangBrands.length === 0 ? (
              <TableRow>
                <TableCell
                  colSpan={3}
      ...
      {/* Form Modal */}
      <BidangBrandFormModal
        isOpen={isModalOpen}
        onOpenChange={setIsModalOpen}
        onSuccess={onFormSuccess}
        initialData={editingBidangBrand || undefined}
      />
    </div>
  );
}
