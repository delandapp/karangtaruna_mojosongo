...
import { KecamatanFormModal } from "../../modals/master/KecamatanFormModal";
import { KecamatanDeleteModal } from "../../modals/master/KecamatanDeleteModal";
...
export function KecamatanTable() {
  ...
  const [editingData, setEditingData] = useState<KecamatanData | null>(null);
  const [deletingData, setDeletingData] = useState<{ id: number; nama: string; kota?: string } | null>(null);
  ...
  const handleProvinsiChange = (val: string) => {
    ...
    setFilterKotaId(""); // Reset kota saat provinsi berubah
    setPage(1);
  };
  ...
  const handleEdit = (data: KecamatanData) => {
    setEditingData(data);
    setIsFormOpen(true);
  };
  ...
  const handleDeleteClick = (id: number, nama: string, kota?: string) => {
    setDeletingData({ id, nama, kota });
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                listData.map((item: KecamatanData) => (
                  <TableRow
                    key={item.id}
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        >
                          ...
                          <DropdownMenuItem
                            onClick={() => handleDeleteClick(item.id, item.nama, item.m_kota?.nama)}
