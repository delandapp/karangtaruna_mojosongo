import { apiSlice } from "@/lib/redux/apiSlice";
import { ApiResponse, ApiQueryParams } from "@/lib/types/api.types";

export const levelApi = apiSlice.injectEndpoints({
    endpoints: (builder) => ({
        getLevels: builder.query<ApiResponse<any[]>, ApiQueryParams | void>({
            query: (params) => {
                let url = "/levels";
                if (params) {
                    const searchParams = new URLSearchParams();
                    if (params.page) searchParams.append("page", params.page.toString());
                    if (params.limit) searchParams.append("limit", params.limit.toString());
                    if (params.search) searchParams.append("search", params.search);
                    if (params.filter?.dropdown) searchParams.append("dropdown", "true");
                    const qs = searchParams.toString();
                    if (qs) url += `?${qs}`;
                }
                return { url, method: "GET" };
            },
            providesTags: ["Level"],
        }),
        createLevel: builder.mutation<ApiResponse<any>, Partial<any>>({
            query: (body) => ({
                url: "/levels",
                method: "POST",
                body,
            }),
            invalidatesTags: ["Level"],
        }),
        updateLevel: builder.mutation<ApiResponse<any>, { id: number; data: Partial<any> }>({
            query: ({ id, data }) => ({
                url: `/levels/${id}`,
                method: "PUT",
                body: data,
            }),
            invalidatesTags: ["Level"],
        }),
        deleteLevel: builder.mutation<ApiResponse<any>, number>({
            query: (id) => ({
                url: `/levels/${id}`,
                method: "DELETE",
            }),
            invalidatesTags: ["Level"],
        }),
    }),
    overrideExisting: false,
});

export const {
    useGetLevelsQuery,
    useCreateLevelMutation,
    useUpdateLevelMutation,
    useDeleteLevelMutation
} = levelApi;
