...
export const createBeritaSchema = z.object({
  ...
  konten_json: z.record(z.string(), z.unknown()), // Tiptap / Editor.js JSON
  ...
  seo_slug: z
    ...
    .regex(
      slugRegex,
      "seo_slug hanya boleh huruf kecil, angka, dan tanda hubung (-)",
    ),
  ...
  seo_schema_json: z.record(z.string(), z.unknown()).optional(), // JSON-LD structured data
});
...
export const createKategoriBeritaSchema = z.object({
  ...
  slug: z
    .string()
    .max(120)
    .regex(slugRegex, "slug hanya huruf kecil, angka, tanda hubung"),
...
export type CreateKategoriBeritaInput = z.infer<
  typeof createKategoriBeritaSchema
>;
