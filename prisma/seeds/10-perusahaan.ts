...
export async function seedPerusahaan(prisma: PrismaClient) {
  ...
  const dataPath = path.join(
    __dirname,
    "../../documentation/data_json/perusahaan.json",
  );

  if (!fs.existsSync(dataPath)) {
    console.warn(
      "File perusahaan.json tidak ditemukan, melewati proses seeding.",
    );
    return;
  }
  ...
  for (const item of data) {
    ...
    if (!kode_provinsi && item.nama_provinsi) {
      if (provinsiCache.has(item.nama_provinsi)) {
        ...
        const p = await prisma.m_provinsi.findFirst({
          where: { nama: item.nama_provinsi },
        });
        if (p?.kode_wilayah) {
    ...
    if (!kode_kota && item.nama_kota) {
      if (kotaCache.has(item.nama_kota)) {
        ...
        const k = await prisma.m_kota.findFirst({
          where: { nama: item.nama_kota },
        });
        if (k?.kode_wilayah) {
    ...
    if (!kode_kecamatan && item.nama_kecamatan) {
      if (kecamatanCache.has(item.nama_kecamatan)) {
        ...
        const k = await prisma.m_kecamatan.findFirst({
          where: { nama: item.nama_kecamatan },
        });
        if (k?.kode_wilayah) {
    ...
    if (!kode_kelurahan && item.nama_kelurahan) {
      if (kelurahanCache.has(item.nama_kelurahan)) {
        ...
        const k = await prisma.m_kelurahan.findFirst({
          where: { nama: item.nama_kelurahan },
        });
        if (k?.kode_wilayah) {
